...
- A C compiler and the `tree-sitter` CLI — compile the configured Treesitter
...
shellcheck, plus LazyVim's configured formatters and Markdown tools.
...
instead of overwriting it. The bootstrap pins `lazy.nvim` itself before startup
and the integration test rejects any lockfile rewrite during first startup.
